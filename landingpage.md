...
import marqueeEnvelope from "@/assets/marquee-envelope.jpg";
import marqueeParty from "@/assets/marquee-party.jpg";
import marqueeValentine from "@/assets/marquee-valentine.jpg";
import {
  BentoCell,
  BentoGrid,
  ContainerScale,
  ...
  return (
    <section id="top" className="relative">
      <ContainerScroll className="h-[350vh]">
        ...
        {/* Scroll-revealed bento gallery — anchored to lower portion so it doesn't sit on the headline */}
        <div className="sticky left-0 top-0 flex h-screen w-full items-end justify-center px-4 pb-10 md:items-center md:pb-0 md:pt-[28vh] md:px-10">
          <BentoGrid className="h-[55vh] w-full max-w-6xl md:h-[62vh]">
            {images.map((src, i) => (
              <BentoCell
                key={i}
        ...
        >
          {steps.map((s) => (
            ...
            >
              ...
              <div className="relative mt-6 aspect-[4/3] overflow-hidden rounded-sm bg-muted">
                <img
                  src={s.img}
                  alt={s.title}
                  loading="lazy"
                  className="h-full w-full scale-110 object-cover opacity-0 blur-md transition-all duration-[900ms] ease-out group-hover:scale-100 group-hover:opacity-100 group-hover:blur-0"
                />
                <div className="absolute inset-0 flex items-center justify-center bg-secondary/30 transition-opacity duration-500 group-hover:opacity-0">
                  <span className="font-display text-xs italic tracking-wider text-muted-foreground">
                    hover to reveal
                  </span>
                </div>
              </div>
